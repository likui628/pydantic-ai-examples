...
@agent.tool_plain(name="todo_write")
...
async def chat(prompt: str) -> str:
    """
    Execute a single agent task with reminder injection and todo tracking.

    Adds soft reminders when needed and tracks whether the LLM used
    the todo_write tool on the last turn so we only nag when helpful.
    """
    global rounds_without_todo, first_message
    ...
    if first_message:
        ...
        reminders.append(NAG_REMINDER)

    full_prompt = "\n".join(reminders + [prompt]) if reminders else prompt

    used_todo = False
    final_output = ""
    tool_call_names: dict[str, str] = {}

    async for event in agent.run_stream_events(full_prompt):
        if isinstance(event, FunctionToolCallEvent):
            tool_call_names[event.part.tool_call_id] = event.part.tool_name
            if event.part.tool_name == "todo_write":
                used_todo = True
...
async def interactive_mode():
    ...
    """
    print(f"Mini Claude Code v2 (todos + reminders) - {WORKDIR}")
